...
function backgroundForType(type: string): string {
  switch (type) {
    case "paper": return "#EDF3FF";
    case "concept": return "#F2EDFF";
    case "method": return "#FFF5DF";
    case "tool": return "#EAF5FC";
    case "dataset": return "#FFF0F7";
    case "result": return "#EAF8F0";
    case "metric": return "#F0EDFF";
    case "sample": return "#E9F8F7";
    case "claim": return "#FFF0EB";
    default: return "#F6F3EC";
  }
}

function KnowledgeNodeComponent({ data, selected }: NodeProps<KnowledgeFlowNode>) {
  ...
  return (
    <div
      className={[
        ...
        selected ? "scale-[1.025] shadow-lg" : "hover:-translate-y-0.5 hover:shadow-md",
    ...
    >
      <Handle id="target-left" type="target" position={Position.Left}
        className="!h-2 !w-2 !border-2 !border-white" style={{ background: accent }} />
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <p className="max-h-10 overflow-hidden text-[12.5px] font-semibold leading-5 text-[#221F1A]"
            title={data.label}>
            {data.label}
          </p>
          <div className="mt-1.5 flex flex-wrap items-center gap-1.5 text-[10px] text-[#726B5C]">
            <span>{readableType(data.nodeType)}</span>
            {confidence !== null && <><span>•</span><span>{confidence}%</span></>}
            {data.connectionCount > 0 && <><span>•</span><span>{data.connectionCount} links</span></>}
          </div>
        </div>
      </div>
