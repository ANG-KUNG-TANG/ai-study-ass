    ...
    <>
      ...
      {!isLoading && !error && notes.length === 0 && (
        <div className="rounded-2xl border border-dashed border-[#E6DDC8] bg-white p-8 text-center">
          <p className="text-[13px] text-[#726B5C]">No notes yet – upload one to generate flashcards.</p>
        </div>
      )}

      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {notes.map((note) => (
          <Link key={note.id} href={`/student/notes/${note.id}/flashcard`}>
            <Card className="cursor-pointer transition hover:-translate-y-1 hover:shadow-lg">
              ...
              <p className="mt-1 text-[13px] text-[#726B5C]">Review flashcards</p>
              <div className="mt-3">
                <span className="rounded-full bg-[#FFF1C2] px-3 py-1 text-[11px] text-[#221F1A]">
                  Flashcards
                </span>
              </div>
            </Card>
