...
const OBJECTIVE_SIGNALS = [
  "objective", "aim", "goal", "provide", "serve", "offer", "deliver", "maintain", "receive", "accept", "prepare",
];
...
export function extractKnowledge(
  ...
  extractObjectiveClaims(nlp.sentences, addClaim);
...
function extractObjectiveClaims(sentences: NLPSentence[], add: ClaimAdder): void {
  const scoped = sentences.filter((sentence) =>
    /objective/i.test(sentence.sectionTitle) || sectionRole(sentence) === "objectives",
  );

  for (const sentence of scoped.slice(0, 12)) {
    const text = sentence.text
      .replace(/^[-–—•*]+\s*/, "")
      .replace(/^\d+[.)]\s*/, "")
      .trim();
    if (text.length < 12 || text.length > 320) continue;
    if (!containsSignal(text, OBJECTIVE_SIGNALS) && scoped.length > 1) continue;
    add({
      type: "objective",
      sentence,
      subject: "Business",
...
function extractDefinitionClaims(sentences: NLPSentence[], add: ClaimAdder): void {
  const sentencePattern = /^(.{2,80}?)\s+(?:is|are|refers to|means)\s+(.{20,260})$/i;
  const glossaryPattern = /^([A-Z][A-Z0-9]{1,10})\s+([A-Z][A-Za-z][A-Za-z /&()-]{3,140})$/;

  for (const sentence of sentences) {
    const glossary = sentence.text.match(glossaryPattern);
    if (glossary && /appendi(?:x|ces)|acronym|terms|definition/i.test(sentence.sectionTitle)) {
      add({
        type: "definition",
        sentence,
        subject: glossary[1].trim(),
        predicate: "means",
        object: glossary[2].trim(),
        confidence: 0.94,
      });
      continue;
    }

    const match = sentence.text.match(sentencePattern);
    if (!match || sentence.text.length > 360) continue;
    const subject = match[1].trim();
    if (!/^[A-Za-z]/.test(subject)) continue;
    if (/^(?:once|when|because|while|after|before|if|as|instead|in|since|although|whereas)\b/i.test(subject)) continue;
    if (subject.split(/\s+/).length > 9) continue;
    const normalizedSubject = subject.toLowerCase().replace(/[^a-z0-9]+/g, " ").trim();
    const subjectSupported = sentence.entities.some((entity) => {
      if (!["CONCEPT", "METHOD", "ACRONYM"].includes(entity.type)) return false;
      const normalizedEntity = entity.text.toLowerCase().replace(/[^a-z0-9]+/g, " ").trim();
      return normalizedSubject === normalizedEntity ||
        normalizedSubject.includes(normalizedEntity) ||
        normalizedEntity.includes(normalizedSubject);
    });
    if (!subjectSupported) continue;
    add({
      type: "definition",
      sentence,
...
function sectionRole(sentence: NLPSentence): string {
  ...
  if (/appendix|acronym|terms/.test(heading)) return "appendix";
  return "background";
}
...
function invalidConceptReason(term: string): string | null {
  ...
  if (/^fr\d+(?:\.\d+)?\b/i.test(value)) return "Requirement identifiers are not concepts.";
  if (/^(?:the\s+)?system\s+(?:shall|must|should|could)\b/i.test(value)) return "Requirement boilerplate is not a domain concept.";
  if (/\b(?:source name|representing|source technique|revised by|priority code|requirement definition)\b/i.test(value)) return "Document metadata is not a domain concept.";
  if (/^(?:once|when|after|before|because|instead|as a result)\b/i.test(value)) return "Discourse fragments are not concepts.";
