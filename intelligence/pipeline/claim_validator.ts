...
function rebuildLegacyCore(core: KnowledgeCore): KnowledgeCore {
  ...
  const keyPoints = buildKeyPoints(valid, core.documentProfile.subtype);
...
function buildKeyPoints(
  claims: ExtractedClaim[],
  subtype?: KnowledgeCore["documentProfile"]["subtype"],
): KeyPoint[] {
  const priorities: ClaimType[] = subtype === "business_requirements_document"
    ? ["problem", "objective", "definition", "method", "result"]
    : ["method", "tool", "sample", "metric", "result", "contribution", "objective"];
  ...
  for (const type of priorities) {
    const max = type === "result" ? 3 : type === "objective" ? 4 : 1;
    const matching = claims.filter((claim) => claim.type === type).slice(0, max);
