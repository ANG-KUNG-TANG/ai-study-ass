...
function requirementsCoverage(
  profile: RequirementsDocumentProfile,
  core: KnowledgeCore,
): { required: string[]; present: string[] } {
  const checks: Array<[string, boolean]> = [
    ["businessProblem", Boolean(core.problem)],
    ["businessObjectives", profile.objectives.length >= 2],
    ["functionalRequirements", profile.requirements.length >= 3],
    ["actors", profile.actors.length >= 2],
    ["processFlow", profile.processSteps.length >= 3],
    ["diagramCoverage", profile.diagramTypes.length >= 2],
    ["priorityScheme", Boolean(profile.priorityScheme)],
  ];
...
function groundingScore(
  ...
  const structuredEvidence = caseStudy
    ? caseStudy.financialInputs.filter((input) => input.evidence.length >= 10).length /
      Math.max(1, caseStudy.financialInputs.length)
    : requirementsDocument
      ? requirementsDocument.requirements.filter((item) => item.evidence.length >= 10).length /
        Math.max(1, requirementsDocument.requirements.length)
      : 1;
  return clamp(conceptEvidence * 0.55 + structuredEvidence * 0.45);
}
...
export function evaluateCoverage(input: {
  ...
  const structural = input.caseStudy && input.classification.domain === "finance"
    ...
    : input.requirementsDocument
      ? requirementsCoverage(input.requirementsDocument, input.core)
  ...
  const grounding = groundingScore(input.concepts, input.caseStudy, input.requirementsDocument);
  ...
  const sectionCoverage = input.classification.kind === "case_study" || input.requirementsDocument
    ? structuralCoverage
    : clamp(
  ...
  if (input.requirementsDocument && missing.includes("functionalRequirements")) {
    criticalWarnings.push("Functional-requirement coverage is too weak for a requirements document.");
  }
  if (input.requirementsDocument && missing.includes("businessObjectives")) {
    criticalWarnings.push("Business-objective coverage is too weak for a requirements document.");
  }
