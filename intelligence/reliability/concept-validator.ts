...
const ALIASES = new Map<string, string>([
  ...
  ["moscow priority coding", "MoSCoW Priority Coding"],
  ["moscow", "MoSCoW"],
]);
...
export function isValidConcept(term: string): boolean {
  ...
  return (
    ...
    !/^(?:once|when|after|before|because|instead|as a result)\b/i.test(normalized) &&
    !/^(?:the\s+)?system\s+(?:shall|must|should|could)\b/i.test(normalized) &&
    !/\b(?:source name|representing|source technique|revised by|requirement id|priority code)\b/i.test(normalized) &&
    !/^fr\d+(?:\.\d+)?\b/i.test(normalized) &&
    !/^\d+(?:\.\d+)?%?$/.test(normalized) &&
    !/[.!?]/.test(normalized) &&
...
export function extractValidatedConcepts(
  ...
  for (const candidate of candidates.sort((a, b) => b.confidence - a.confidence)) {
    ...
    const subsumed = output.some((existing) => {
      const existingWords = existing.normalized.split(/\s+/).filter(Boolean);
      return existingWords.length > normalizedWords.length &&
...
export function extractValidatedKeyTerms(
  ...
  for (const sentence of sentences) {
    const glossary = sentence.match(/^([A-Z][A-Z0-9]{1,10})\s+([A-Z][A-Za-z][A-Za-z /&()'’-]{3,160})$/);
    if (glossary) {
      const term = glossary[1].trim();
      const definition = cleanDefinition(glossary[2]);
      const key = term.toLowerCase();
      const metadataLike = term === "ID" && /\b(?:name|approve|approval|date|signature)\b/i.test(definition);
      if (!metadataLike && !seen.has(key) && definition.split(/\s+/).length >= 2) {
        seen.add(key);
        definitions.push({
          term,
          definition,
    ...
    if (/^(?:once|when|because|while|after|before|if|as|instead|since|although|whereas)\b/i.test(rawTerm)) continue;
    ...
    const strongDefinitionCue = /\b(?:means|refers to|is defined as)\b/i.test(sentence);
    if (!conceptMatch && !strongDefinitionCue) continue;
